...
const requireLogin = () => __async(exports, null, function* () {
  const userInfo = getCurrentUserInfo();
...
function classifyServiceByName(name) {
...
const symptomService = {
  ...
  getTherapistSymptomServices(therapistId) {
    return __async(this, null, function* () {
      ...
      try {
        const therapistResponse = yield request(`/therapists/${therapistId}`);
        ...
        const storeData = yield request(`/stores/${therapist.storeId}`);
        const services = storeData.data.services || [];
        const categorizedServices = services.map((service) => __spreadProps(__spreadValues({}, service), {
  ...
  getStoreSymptomServices(storeId) {
    return __async(this, null, function* () {
      ...
      try {
        const storeData = yield request(`/stores/${storeId}`);
        const services = storeData.data.services || [];
        const categorizedServices = services.map((service) => __spreadProps(__spreadValues({}, service), {
          categoryId: classifyServiceByName(service.name),
          availability: "available",
  ...
  getServicesByCategory(therapistId, categoryId) {
    return __async(this, null, function* () {
      ...
      const allServicesResponse = yield this.getTherapistSymptomServices(therapistId);
...
class TherapistService {
  ...
  // 获取门店所有推拿师的排班信息
  // 用于症状页面显示技师可用性
  getTherapistsAvailability(storeId, date, days = 3) {
    return __async(this, null, function* () {
      try {
        const data = yield request(`/stores/${storeId}/therapists/availability`, {
          data: {
            date,
            days
          }
...
class OrderService {
    ...
    return __async(this, null, function* () {
      try {
        ...
        const requestData = {
          ...
          price: (params.discountPrice || params.price) * 100,
          // ✅ 转换为分（params中是元）
          paymentMethod: params.paymentMethod
          // ✅ 新增：传递支付方式
        };
  ...
  getPaymentParams(orderNo) {
    return __async(this, null, function* () {
      var _a, _b, _c;
      try {
        const response = yield get(`/orders/${orderNo}/payment-params`);
        ...
        if (response.data.wxPayParams) {
          ...
          console.log("💳 微信支付参数已提取:", {
            ...
            nonceStr: ((_a = paymentParams.nonceStr) == null ? void 0 : _a.substring(0, 8)) + "...",
            package: paymentParams.package,
            signType: paymentParams.signType,
            paySign: ((_b = paymentParams.paySign) == null ? void 0 : _b.substring(0, 16)) + "...",
        ...
        console.error("💳 错误详情:", {
          ...
          response: (_c = error.response) == null ? void 0 : _c.data
        });
