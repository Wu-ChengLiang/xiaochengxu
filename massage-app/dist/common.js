...
class OrderService {
      ...
      try {
        const response = yield taro.Taro.request({
          ...
          data: {
            ...
            page: 1,
            pageSize: 100,
            status: status ? this.unmapStatus(status) : void 0
          },
          header: {
        ...
        return result.data.list.map((order) => this.transformApiOrder(order));
      } catch (error) {
  ...
  transformApiOrder(apiOrder) {
    ...
    const baseOrder = {
      ...
      totalAmount: apiOrder.amount / 100,
      // 分转元
    ...
    if (apiOrder.orderType === "service") {
      return __spreadProps(__spreadValues({}, baseOrder), {
        // 从extraData提取按摩预约字段
        ...
        discountPrice: extraData.discountPrice ? extraData.discountPrice / 100 : void 0
      });
