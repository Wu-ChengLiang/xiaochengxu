...
const wechatLogin = () => __async(exports, null, function* () {
  try {
    ...
    const response = yield post("/api/v2/users/wechat-login", {
      code
    });
    if (response.data) {
...
const bindPhone = (openid, phone) => __async(exports, null, function* () {
  try {
    const response = yield post("/api/v2/users/bind-phone", {
      openid,
      phone
    });
    if (response.data) {
...
const fetchUserInfo = (phone) => __async(exports, null, function* () {
  try {
    ...
    const response = yield get(`/api/v2/users/info?phone=${phone}`);
    if (response.data) {
      const userInfo = {
        id: response.data.id,
