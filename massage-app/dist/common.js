...
const API_CONFIG = {
  baseURL: "http://emagen.323424.xyz",
  useMock: false
...
class StoreService {
  ...
  getNearbyStores(latitude, longitude, page = 1, pageSize = 10) {
    return __async(this, null, function* () {
      try {
        const data = yield request("/api/v2/stores/nearby", {
          data: {
            latitude,
            longitude,
            page,
  ...
  getStoreDetail(storeId) {
    return __async(this, null, function* () {
      try {
        const data = yield request(`/api/v2/stores/${storeId}`);
  ...
  searchStores(keyword, page = 1, pageSize = 10) {
    return __async(this, null, function* () {
      try {
        const data = yield request("/api/v2/stores/search", {
          data: {
            keyword,
            page,
            pageSize
  ...
  getStoresByStatus(status, page = 1, pageSize = 10) {
    return __async(this, null, function* () {
      try {
        const data = yield request("/api/v2/stores/filter", {
          data: {
            status,
            page,
            pageSize
...
class TherapistService {
  ...
  getRecommendedTherapists(page = 1, pageSize = 10, userLocation) {
    return __async(this, null, function* () {
      try {
        const data = yield request("/api/v2/therapists/recommended", {
          data: {
            page,
            pageSize,
...
class WalletService {
  ...
  getBalance() {
    return __async(this, null, function* () {
      try {
        ...
        const response = yield request(`/api/v2/users/wallet/balance?userId=${userId}`);
  ...
  // 充值（创建充值订单）
  recharge(amount, description = "余额充值") {
    return __async(this, null, function* () {
      try {
        if (amount <= 0) {
          return {
        ...
        const userId = this.getCurrentUserId();
        const orderResponse = yield request("/api/v2/orders/create", {
          method: "POST",
          data: {
            orderType: "recharge",
            userId,
            ...
            extraData: {
              bonus: this.calculateBonus(amount * 100)
              // 计算赠送金额
            }
          }
        });
  ...
  // 消费（创建订单并支付）
  consume(amount, description, orderId) {
    return __async(this, null, function* () {
      try {
        if (amount <= 0) {
          return {
        ...
        const payResponse = yield request("/api/v2/orders/pay", {
          method: "POST",
          data: {
            orderNo: orderId,
            paymentMethod: "balance"
        ...
        return {
          success: true,
          balance: (payResponse.data.balance || 0) / 100,
          // 转换为元
          message: "支付成功",
          transaction: {
            id: `PAY_${orderId}`,
  ...
  // 退款
  refund(amount, description = "订单退款", orderId) {
    return __async(this, null, function* () {
      try {
        if (amount <= 0) {
          return {
        ...
        const userId = this.getCurrentUserId();
        const response = yield request("/api/v2/users/wallet/refund", {
          method: "POST",
          data: {
            phone: yield this.getUserPhone(),
            amount: amount * 100,
            // 转换为分
            orderNo: orderId,
            description
  ...
  getTransactions(page = 1, pageSize = 20) {
    return __async(this, null, function* () {
      try {
        const userId = this.getCurrentUserId();
        const response = yield request(`/api/v2/users/wallet/transactions?userId=${userId}&page=${page}&pageSize=${pageSize}`);
        console.log("✅ 获取交易记录成功:", response);
        const transactions = response.data.list.map((item) => ({
          id: item.id,
          type: item.type,
