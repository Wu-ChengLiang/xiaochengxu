  ...
  const handleCancel = () => __async(exports, null, function* () {
    taro.Taro.showModal({
      ...
      success: (res) => __async(exports, null, function* () {
        if (res.confirm) {
          try {
            const result = yield common.orderService.cancelOrder(orderNo);
            if ((orderInfo == null ? void 0 : orderInfo.paymentStatus) === "pending") {
              taro.Taro.showToast({
                title: "取消订单",
                icon: "success"
              });
            } else if ((orderInfo == null ? void 0 : orderInfo.paymentStatus) === "paid" && result.refundAmount && result.refundAmount > 0) {
              taro.Taro.showToast({
                title: `取消订单`,
                icon: "success"
              });
