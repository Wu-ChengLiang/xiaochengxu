"use strict";
var __async = (__this, __arguments, generator) => {
  return new Promise((resolve, reject) => {
    var fulfilled = (value) => {
      try {
        step(generator.next(value));
      } catch (e) {
        reject(e);
      }
    };
    var rejected = (value) => {
      try {
        step(generator.throw(value));
      } catch (e) {
        reject(e);
      }
    };
    var step = (x) => x.done ? resolve(x.value) : Promise.resolve(x.value).then(fulfilled, rejected);
    step((generator = generator.apply(__this, __arguments)).next());
  });
};
const taro = require("../../taro.js");
const vendors = require("../../vendors.js");
const common = require("../../common.js");
const index$2 = "";
const PhoneAuth = ({
  type,
  openid,
  onSuccess,
  onCancel,
  buttonText,
  buttonSize = "default",
  buttonType = "primary"
}) => {
  const handleGetPhoneNumber = (e) => __async(exports, null, function* () {
    var _a, _b;
    console.log("手机号授权回调:", e.detail);
    console.log("获取到code:", e.detail.code);
    console.log("当前时间:", (/* @__PURE__ */ new Date()).toISOString());
    console.log("当前openid:", openid);
    if (e.detail.errMsg === "getPhoneNumber:ok") {
      const { code } = e.detail;
      if (!code) {
        taro.Taro.showToast({
          title: "获取手机号失败",
          icon: "none"
        });
        return;
      }
      console.log("准备发送请求，code:", code);
      console.log("请求时间:", (/* @__PURE__ */ new Date()).toISOString());
      try {
        taro.Taro.showLoading({ title: "处理中..." });
        let result;
        if (type === "bind") {
          if (!openid) {
            throw new Error("缺少openid参数");
          }
          console.log("发送绑定请求，参数:", { openid, code });
          const response = yield common.post("/users/bind-phone-wx", {
            openid,
            code
          });
          console.log("绑定响应:", response);
          result = response.data;
          if (result.phone) {
            const userInfo = yield common.fetchUserInfo(result.phone);
            if (userInfo) {
              common.setUserInfo(userInfo);
            }
          }
        } else {
          const currentUser = common.getCurrentUserInfo();
          if (!currentUser) {
            throw new Error("请先登录");
          }
          const response = yield common.post("/users/change-phone-wx", {
            userId: currentUser.id,
            oldPhone: currentUser.phone,
            code
          });
          result = response.data;
          if (result.phone) {
            const userInfo = yield common.fetchUserInfo(result.phone);
            if (userInfo) {
              common.setUserInfo(userInfo);
            }
          }
        }
        taro.Taro.hideLoading();
        if (result && result.phone) {
          taro.Taro.showToast({
            title: type === "bind" ? "绑定成功" : "换绑成功",
            icon: "success"
          });
          onSuccess == null ? void 0 : onSuccess(result.phone);
        } else {
          throw new Error("操作失败");
        }
      } catch (error) {
        taro.Taro.hideLoading();
        console.error("手机号操作失败:", error);
        console.error("错误详情:", {
          message: error.message,
          response: error.response,
          status: (_a = error.response) == null ? void 0 : _a.status,
          data: (_b = error.response) == null ? void 0 : _b.data
        });
        const errorMsg = error.message || (type === "bind" ? "绑定失败" : "换绑失败");
        taro.Taro.showToast({
          title: errorMsg,
          icon: "none",
          duration: 2e3
        });
      }
    } else if (e.detail.errMsg === "getPhoneNumber:fail user deny") {
      console.log("用户拒绝授权手机号");
      if (type === "bind") {
        taro.Taro.showModal({
          title: "温馨提示",
          content: "需要您的手机号才能为您提供预约服务",
          showCancel: true,
          confirmText: "知道了",
          success: (res) => {
            if (res.cancel) {
              onCancel == null ? void 0 : onCancel();
            }
          }
        });
      } else {
        taro.Taro.showToast({
          title: "已取消换绑",
          icon: "none"
        });
        onCancel == null ? void 0 : onCancel();
      }
    } else {
      console.error("获取手机号失败:", e.detail.errMsg);
      taro.Taro.showToast({
        title: "获取手机号失败",
        icon: "none"
      });
    }
  });
  const getButtonText = () => {
    if (buttonText)
      return buttonText;
    if (type === "bind") {
      return buttonSize === "mini" ? "绑定" : "一键绑定手机号";
    } else {
      return buttonSize === "mini" ? "更换" : "更换手机号";
    }
  };
  return /* @__PURE__ */ taro.jsx(taro.View, { className: "phone-auth-component", children: /* @__PURE__ */ taro.jsx(
    taro.Button,
    {
      className: `phone-auth-btn phone-auth-btn--${buttonSize}`,
      type: buttonType,
      size: buttonSize,
      openType: "getPhoneNumber",
      onGetPhoneNumber: handleGetPhoneNumber,
      children: getButtonText()
    }
  ) });
};
const index$1 = "";
const NewUserVoucherModal = ({
  isOpened,
  discountPercentage,
  onClose,
  onUse
}) => {
  const handleUse = () => {
    onUse();
    taro.Taro.switchTab({
      url: "/pages/appointment/index"
    });
  };
  const handleViewVouchers = () => {
    onClose();
    taro.Taro.navigateTo({
      url: "/pages/mine/vouchers/index"
    });
  };
  return /* @__PURE__ */ taro.jsx(vendors.AtModal, { isOpened, closeOnClickOverlay: false, children: /* @__PURE__ */ taro.jsx(vendors.AtModalContent, { children: /* @__PURE__ */ taro.jsxs(taro.View, { className: "new-user-voucher-modal", children: [
    /* @__PURE__ */ taro.jsxs(taro.View, { className: "celebration-bg", children: [
      /* @__PURE__ */ taro.jsx(taro.View, { className: "confetti confetti-1", children: "🎉" }),
      /* @__PURE__ */ taro.jsx(taro.View, { className: "confetti confetti-2", children: "🎊" }),
      /* @__PURE__ */ taro.jsx(taro.View, { className: "confetti confetti-3", children: "✨" }),
      /* @__PURE__ */ taro.jsx(taro.View, { className: "confetti confetti-4", children: "🎁" })
    ] }),
    /* @__PURE__ */ taro.jsxs(taro.View, { className: "modal-header", children: [
      /* @__PURE__ */ taro.jsx(taro.Text, { className: "welcome-text", children: "欢迎新用户" }),
      /* @__PURE__ */ taro.jsx(taro.Text, { className: "congrats-text", children: "恭喜获得专属优惠券！" })
    ] }),
    /* @__PURE__ */ taro.jsx(taro.View, { className: "voucher-display", children: /* @__PURE__ */ taro.jsxs(taro.View, { className: "voucher-card-preview", children: [
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "discount-area", children: [
        /* @__PURE__ */ taro.jsxs(taro.Text, { className: "discount-value", children: [
          discountPercentage,
          "%"
        ] }),
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "discount-label", children: "OFF" })
      ] }),
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "voucher-details", children: [
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "voucher-title", children: "新人专享券" }),
        /* @__PURE__ */ taro.jsxs(taro.Text, { className: "voucher-subtitle", children: [
          "全场服务",
          100 - discountPercentage,
          "折"
        ] }),
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "voucher-desc", children: "适用于所有推拿服务" })
      ] })
    ] }) }),
    /* @__PURE__ */ taro.jsxs(taro.View, { className: "benefits-list", children: [
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "benefit-item", children: [
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-icon", children: "✅" }),
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-text", children: "所有服务均可使用" })
      ] }),
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "benefit-item", children: [
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-icon", children: "✅" }),
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-text", children: "自动应用最优折扣" })
      ] }),
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "benefit-item", children: [
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-icon", children: "✅" }),
        /* @__PURE__ */ taro.jsx(taro.Text, { className: "benefit-text", children: "有效期长达一年" })
      ] })
    ] }),
    /* @__PURE__ */ taro.jsxs(taro.View, { className: "modal-actions", children: [
      /* @__PURE__ */ taro.jsx(taro.View, { className: "primary-btn", onClick: handleUse, children: "立即使用" }),
      /* @__PURE__ */ taro.jsx(taro.View, { className: "secondary-btn", onClick: handleViewVouchers, children: "查看我的券包" }),
      /* @__PURE__ */ taro.jsx(taro.View, { className: "close-btn", onClick: onClose, children: "稍后再说" })
    ] })
  ] }) }) });
};
const LogoImg = "data:image/png;base64,";
const index = "";
const Mine = () => {
  const [balance, setBalance] = taro.useState(0);
  const [userInfo, setUserInfo] = taro.useState(null);
  const [loading, setLoading] = taro.useState(false);
  const [locationEnabled, setLocationEnabled] = taro.useState(false);
  const [showNewUserVoucher, setShowNewUserVoucher] = taro.useState(false);
  const [voucherInfo, setVoucherInfo] = taro.useState({
    discountPercentage: 0
  });
  const [showBindPhoneModal, setShowBindPhoneModal] = taro.useState(false);
  const [phoneInput, setPhoneInput] = taro.useState("");
  taro.useState(false);
  const [currentOpenid, setCurrentOpenid] = taro.useState("");
  const isInitialLoad = taro.react_production_min.useRef(true);
  taro.useEffect(() => {
    initUser();
    checkLocation();
  }, []);
  taro.taroExports.useDidShow(() => {
    if (isInitialLoad.current) {
      isInitialLoad.current = false;
      return;
    }
    checkLocation();
  });
  const checkLocation = () => __async(exports, null, function* () {
    try {
      yield common.getLocationService.getCurrentLocation();
      setLocationEnabled(true);
    } catch (error) {
      setLocationEnabled(false);
    }
  });
  const initUser = () => __async(exports, null, function* () {
    setLoading(true);
    try {
      const localUser = common.getCurrentUserInfo();
      if (localUser && localUser.phone) {
        setUserInfo(localUser);
        fetchBalance();
        common.checkAndAutoLogin().then((freshUser) => {
          if (freshUser) {
            setUserInfo(freshUser);
          }
        });
      } else {
        const userInfo2 = yield common.checkAndAutoLogin();
        if (userInfo2) {
          setUserInfo(userInfo2);
          fetchBalance();
        } else {
          console.log("需要用户手动登录");
          setUserInfo(null);
        }
      }
    } catch (error) {
      console.error("初始化用户失败:", error);
      setUserInfo(null);
    } finally {
      setLoading(false);
    }
  });
  const fetchBalance = () => __async(exports, null, function* () {
    try {
      const currentBalance = yield common.walletService.getBalance();
      setBalance(currentBalance);
    } catch (error) {
      console.error("获取余额失败:", error);
    }
  });
  const menuItems = [
    {
      icon: "file-generic",
      title: "我的订单",
      path: "/pages/order/list/index",
      arrow: true
    },
    {
      icon: "money",
      title: "我的券包",
      path: "/pages/mine/vouchers/index",
      arrow: true
    },
    // {
    //   icon: 'gift',
    //   title: '邀请有奖',
    //   path: '/pages/mine/invite/index',
    //   arrow: true
    // },
    // {
    //   icon: 'phone',
    //   title: '联系客服',
    //   path: '/pages/mine/contact/index',
    //   arrow: true
    // },
    {
      icon: "help",
      title: "关于我们",
      path: "/pages/mine/about/index",
      arrow: true
    }
  ];
  const handleMenuClick = (item) => {
    taro.Taro.navigateTo({ url: item.path });
  };
  const handleLogin = () => __async(exports, null, function* () {
    if (loading)
      return;
    setLoading(true);
    try {
      const loginResult = yield common.wechatLogin();
      if (loginResult.needBindPhone) {
        setCurrentOpenid(loginResult.openid);
        setShowBindPhoneModal(true);
      } else if (loginResult.userInfo) {
        setUserInfo(loginResult.userInfo);
        fetchBalance();
        checkAndShowNewUserVoucher();
        taro.Taro.showToast({
          title: "登录成功",
          icon: "success"
        });
      }
    } catch (error) {
      console.error("微信登录失败:", error);
      taro.Taro.showToast({
        title: "登录失败，请重试",
        icon: "error"
      });
    } finally {
      setLoading(false);
    }
  });
  const handleCancelBindPhone = () => {
    setShowBindPhoneModal(false);
    setPhoneInput("");
    setCurrentOpenid("");
  };
  const handleBalanceClick = () => {
    taro.Taro.navigateTo({
      url: "/pages/mine/balance/index"
    });
  };
  const checkAndShowNewUserVoucher = () => {
    const voucherInfo2 = common.voucherService.getNewUserVoucherInfo();
    if (voucherInfo2.hasVoucher && voucherInfo2.discountPercentage) {
      const hasShown = taro.Taro.getStorageSync("newUserVoucherShown");
      if (!hasShown) {
        setVoucherInfo({ discountPercentage: voucherInfo2.discountPercentage });
        setTimeout(() => {
          setShowNewUserVoucher(true);
        }, 1500);
      }
    }
  };
  const handleCloseNewUserVoucher = () => {
    setShowNewUserVoucher(false);
    taro.Taro.setStorageSync("newUserVoucherShown", true);
  };
  const handleUseNewUserVoucher = () => {
    setShowNewUserVoucher(false);
    taro.Taro.setStorageSync("newUserVoucherShown", true);
  };
  return /* @__PURE__ */ taro.jsxs(taro.View, { className: "mine-page", children: [
    /* @__PURE__ */ taro.jsx(taro.View, { className: "header-section", children: /* @__PURE__ */ taro.jsxs(taro.View, { className: "user-card", children: [
      /* @__PURE__ */ taro.jsxs(taro.View, { className: "user-info", children: [
        /* @__PURE__ */ taro.jsx(
          taro.Image,
          {
            className: "avatar",
            src: (userInfo == null ? void 0 : userInfo.avatar) || LogoImg,
            mode: "aspectFill"
          }
        ),
        userInfo ? /* @__PURE__ */ taro.jsx(taro.Text, { className: "phone", children: userInfo.nickname || userInfo.username || common.maskPhone(userInfo.phone) }) : /* @__PURE__ */ taro.jsx(
          taro.Button,
          {
            className: "login-btn",
            onClick: handleLogin,
            loading,
            disabled: loading,
            size: "mini",
            type: "primary",
            children: loading ? "登录中..." : "微信登录"
          }
        )
      ] }),
      userInfo && /* @__PURE__ */ taro.jsxs(taro.Fragment, { children: [
        /* @__PURE__ */ taro.jsxs(taro.View, { className: "phone-info", children: [
          /* @__PURE__ */ taro.jsx(taro.Text, { className: "phone-label", children: "手机号: " }),
          /* @__PURE__ */ taro.jsx(taro.Text, { className: "phone-number", children: common.maskPhone(userInfo.phone) }),
          /* @__PURE__ */ taro.jsx(
            PhoneAuth,
            {
              type: "change",
              buttonText: "换绑",
              buttonSize: "mini",
              buttonType: "default",
              onSuccess: (phone) => {
                console.log("换绑成功，新手机号:", phone);
                taro.Taro.showToast({
                  title: "换绑成功",
                  icon: "success"
                });
                initUser();
              }
            }
          )
        ] }),
        /* @__PURE__ */ taro.jsxs(taro.View, { className: "balance-location-row", children: [
          /* @__PURE__ */ taro.jsxs(taro.View, { className: "balance-info", onClick: handleBalanceClick, children: [
            /* @__PURE__ */ taro.jsx(taro.Text, { className: "balance-label", children: "余额: " }),
            /* @__PURE__ */ taro.jsxs(taro.Text, { className: "balance-amount", children: [
              "¥ ",
              balance.toFixed(2)
            ] }),
            /* @__PURE__ */ taro.jsx(vendors.AtIcon, { value: "chevron-right", size: "16", color: "#fff" })
          ] }),
          /* @__PURE__ */ taro.jsx(taro.View, { className: "location-info", children: /* @__PURE__ */ taro.jsx(taro.Text, { className: "location-text", children: locationEnabled ? "定位已启用" : "定位未启用" }) })
        ] })
      ] })
    ] }) }),
    /* @__PURE__ */ taro.jsx(taro.View, { className: "menu-section", children: menuItems.map(
      (item, index2) => /* @__PURE__ */ taro.jsxs(
        taro.View,
        {
          className: "menu-item",
          onClick: () => handleMenuClick(item),
          children: [
            /* @__PURE__ */ taro.jsxs(taro.View, { className: "menu-left", children: [
              /* @__PURE__ */ taro.jsx(
                vendors.AtIcon,
                {
                  prefixClass: "icon",
                  value: item.icon,
                  size: "20",
                  color: "#666"
                }
              ),
              /* @__PURE__ */ taro.jsx(taro.Text, { className: "menu-title", children: item.title })
            ] }),
            /* @__PURE__ */ taro.jsxs(taro.View, { className: "menu-right", children: [
              item.badge && /* @__PURE__ */ taro.jsx(taro.View, { className: "badge", children: item.badge }),
              item.arrow && /* @__PURE__ */ taro.jsx(vendors.AtIcon, { value: "chevron-right", size: "16", color: "#999" })
            ] })
          ]
        },
        index2
      )
    ) }),
    /* @__PURE__ */ taro.jsxs(
      vendors.AtModal,
      {
        isOpened: showBindPhoneModal,
        onCancel: handleCancelBindPhone,
        children: [
          /* @__PURE__ */ taro.jsx(vendors.AtModalHeader, { children: "绑定手机号" }),
          /* @__PURE__ */ taro.jsx(vendors.AtModalContent, { children: /* @__PURE__ */ taro.jsxs(taro.View, { className: "bind-phone-content", children: [
            /* @__PURE__ */ taro.jsx(taro.Text, { className: "bind-phone-tips", children: "使用微信绑定的手机号快速登录" }),
            /* @__PURE__ */ taro.jsx(taro.View, { className: "phone-auth-wrapper", children: /* @__PURE__ */ taro.jsx(
              PhoneAuth,
              {
                type: "bind",
                openid: currentOpenid,
                onSuccess: (phone) => {
                  console.log("绑定成功，手机号:", phone);
                  setShowBindPhoneModal(false);
                  setPhoneInput("");
                  const latestUserInfo = common.getCurrentUserInfo();
                  if (latestUserInfo) {
                    setUserInfo(latestUserInfo);
                    fetchBalance();
                  }
                },
                onCancel: () => {
                  console.log("用户取消绑定");
                }
              }
            ) }),
            /* @__PURE__ */ taro.jsx(taro.Text, { className: "bind-phone-privacy", children: "授权后将获取您的微信手机号用于账号登录" })
          ] }) }),
          /* @__PURE__ */ taro.jsx(vendors.AtModalAction, { children: /* @__PURE__ */ taro.jsx(taro.Button, { onClick: handleCancelBindPhone, children: "稍后绑定" }) })
        ]
      }
    ),
    /* @__PURE__ */ taro.jsx(
      NewUserVoucherModal,
      {
        isOpened: showNewUserVoucher,
        discountPercentage: voucherInfo.discountPercentage,
        onClose: handleCloseNewUserVoucher,
        onUse: handleUseNewUserVoucher
      }
    )
  ] });
};
var config = {
  "usingComponents": {
    "comp": "../../comp"
  }
};
Page(taro.createPageConfig(Mine, "pages/mine/index", { root: { cn: [] } }, config || {}));
//# sourceMappingURL=index.js.map
